...
public class PathSearch {
	...
	public enum Directions {
		SQUARE_FOUR_DIR, SQUARE_EIGHT_DIR, HEX_SIX_DIR, TRIANGLE_TWELVE_DIR
	}

	public enum SearchMethod {
		BFS, DFS, GREEDY, UNIFORM, ASTAR
	}
	...
	private static final int[][] DIRS_4 = { { 0, -1 }, // up
			{ 0, 1 }, // down
			{ -1, 0 }, // left
			{ 1, 0 } // right
	};
	private static final int[][] DIRS_8 = { { 0, -1 }, // up
			{ 0, 1 }, // down
			{ -1, 0 }, // left
			{ 1, 0 }, // right
			{ -1, -1 }, { -1, 1 }, { 1, -1 }, { 1, 1 } };

	private static final int[][] DIRS_TRI_EVEN = { { 0, -1 }, // up
			{ 1, -1 }, // up right
			{ -1, -1 }, // up left
			{ 0, 1 }, // down
			{ 1, 0 }, // right
	...
	private static final int[][] DIRS_TRI_ODD = { { 0, 1 }, { 1, 0 }, { -1, 0 }, { 1, -1 }, { -1, -1 }, { 2, 0 },
			{ -2, 0 }, { 1, 1 }, { -1, 1 }, { 0, -1 }, { 2, -1 }, { -2, -1 } };

	// Axial directions for pointy-top hex grid
	static final int[][] AXIAL_DIRS = { { +1, 0 }, { +1, -1 }, { 0, -1 }, { -1, 0 }, { -1, +1 }, { 0, +1 } };
			...
			for (int y = 0; y < TileGrid.GetHeight(); y++) {
				...
				case HEX_SIX_DIR -> {
					...
					for (int[] d : AXIAL_DIRS) {
						int nq = currentTile.q + d[0];
						int nr = currentTile.r + d[1];

						// Convert axial → offset
						int dx = nq;
						int dy = nr + ((nq - (nq & 1)) / 2);

						if (dx >= 0 && dx < TileGrid.GetWidth() && dy >= 0 && dy < TileGrid.GetHeight()) {

							Tile neighborTile = TileGrid.GetTile(dx, dy);

							if (neighborTile != null && neighborTile.walkable && Nodes.containsKey(neighborTile)
									&& neighborTile.weight > 0 && !neighborTile.blocked) {
	...
	public void Enter(int startX, int startY, int goalX, int goalY) {
		...
		startNode.finalCost = calculateFinalCost(startNode.givenCost, startNode.heuristicCost,
				startNode.searchNode.tile.weight);
	...
	private boolean BreathFirst() {
		...
		for (SearchNode neighborNode : currentNode.searchNode.neighbors) {
			if (!neighborNode.tile.walkable)
				continue;
			if (neighborNode.tile.inClosedSet)
				continue;
	...
	public boolean AStarSearch() {
		...
		for (SearchNode neighborNode : currentNode.searchNode.neighbors) {
			...
			nextNode.givenCost = newCost;
			nextNode.heuristicCost = heuristic(nextNode.searchNode.tile, GoalTile.tile);
			nextNode.finalCost = calculateFinalCost(nextNode.givenCost, nextNode.heuristicCost,
					nextNode.searchNode.tile.weight);
			neighborNode.tile.inOpenSet = true;
			VisitedNodes.put(neighborNode, nextNode);
			openPriorityQueue.add(nextNode);
		}
